...
async function handle(request, env) {
  ...
  if (!originAllowed(request, env)) return json({ code: 'ORIGIN_NOT_ALLOWED', message: 'This origin is not allowed to use the map proxy.' }, 403, cors)
  if (request.method === 'OPTIONS') return new Response(null, { status: 204, headers: cors })
  if (!allowRequest(request, Number(env.MAX_REQUESTS_PER_MINUTE) || 60)) {
    return json({ code: 'RATE_LIMITED', message: 'Too many location requests. Please wait a moment.' }, 429, cors)
  }
  ...
  try {
    ...
    // Backward-compatible alias for v3.0.2 clients. New frontend code uses /search?intent=airport.
      ...
      response = json({
        ...
        build: 'refactor-pass-1',
        routes: ['/api/maps/nearby', '/api/maps/route', '/api/maps/transit/plan', '/api/maps/transit/stops'],
      ...
      const transit = transitBaseUrl(request, env)
      response = json({ ok: true, version: WORKER_VERSION, build: 'refactor-pass-1', provider: 'heigit', transitProvider: transit.provider || 'not-configured', airportSearch: 'wikidata-only', poiMaxGroupsPerRequest: 5, transitEndpoints: true, paidFallback: false })
    }
    ...
    }, error?.status === 429 ? 429 : error?.status === 401 || error?.status === 403 ? error.status : 503, cors)
  }
}
